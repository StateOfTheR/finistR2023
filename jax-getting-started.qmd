...
Avant de les multipliers. On utilise la fonction `block_until_ready()` pour mesurer le temps effectif de calcul. En effet, JAX fait de l'évaluation asynchrone (comme {{future}} en R) pour rendre la main à l'utilisateur après l'envoi de la commande. 

```{python}
%time
odds = jnp.dot(x, beta_true)  # runs on the CPU
```
...
mais on peut l'accélerer d'un facteur 2 ou 3 en compilant la fonction via `jit()` de façon complètement transparente pour l'utilisateur. 
