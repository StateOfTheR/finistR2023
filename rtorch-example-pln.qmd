...
On charge le jeu de données `oaks` contrnu dans `PLNmodels`
...
On définit une simple classe avec une routine d'optimisation utilisant `Rprop` et l'auto-differentiation sur l'le critère à optimiser (l'ELBO = Expected Lower BOund).
...
On crée une instance avec les données appropriée
...
plot(-myPLN$ELBO_list, type="l", log ='y')
```
...
```{r}
plot(myPLN$B,
     myPLN_nlopt$model_par$B); abline(0, 1)
plot(myPLN_torch$model_par$Sigma,
     myPLN$Sigma); abline(0, 1)
```