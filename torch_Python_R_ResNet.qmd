---
title: "Comparaison {torch} (`R`) et {pytorch}"
format: html
editor: visual
execute:
  cache: true
...
transform <- function(img) {
  # img <- torchvision::transform_pad(img, 4)
  # img <- torchvision::transform_random_horizontal_flip(img)
  # img <- torchvision::transform_random_crop(img, 32)
...
train_loader <- torch::dataloader(
  dataset = torch::dataset_subset(train_data, 1:num_samples),
  batch_size = 256,
  shuffle = TRUE
)

test_loader <- torch::dataloader(
  dataset = torch::dataset_subset(test_data, 1:num_samples),
  batch_size = 256,
  shuffle = FALSE
)
```
...
De la même manière, on peut créer un objet `torch::nn_module` en `R` et y spécifier `init` et `forward` :
```{r}
...
res_block <- torch::nn_module(
  initialize = function(num_in, num_out, stride) {
    self$align <- align(num_in, num_out, stride)
    self$conv1 <- torch::nn_conv2d(num_in, num_out,
      kernel_size = 3,
      stride = stride, padding = 1, bias = FALSE
    )
    self$bn1 <- torch::nn_batch_norm2d(num_out)
    self$relu <- torch::nn_relu(inplace = TRUE)
    self$conv2 <- torch::nn_conv2d(num_out, num_out,
      kernel_size = 3,
      stride = 1, padding = 1, bias = FALSE
    )
    self$bn2 <- torch::nn_batch_norm2d(num_out)
  },
  forward = function(x) {
    o <- self$conv1(x)
    o <- self$bn1(o)
...
build_res_blocks <- function(num_in, num_out, stride, num_blocks) {
  ...
  return(do.call(torch::nn_sequential, blocks))
}

res_net <- torch::nn_module(
  initialize = function(num_classes) {
    self$blocks0 <- torch::nn_sequential(
      torch::nn_conv2d(
        3,
        16,
        kernel_size = 3,
    ...
    self$avgpool <- torch::nn_avg_pool2d(kernel_size = 8)
    self$fc <- torch::nn_linear(64, num_classes)
  },
  forward = function(x) {
    n <- dim(x)[1]
    o <- self$blocks0(x)
    ...
    o <- self$blocks3(o)
    o <- self$avgpool(o)
    o <- torch::torch_flatten(o, start_dim = 2)
    o <- self$fc(o)
    return(o)
  }
)
...
train <- function() {
  for (epoch in 1:10) {
    ...
    coro::loop(for (batch in train_loader) {
      ...
      y <- batch[[2]]
      o <- model(x)
      loss <- torch::nnf_cross_entropy(o, y)
...
torch::with_no_grad({
  ...
  coro::loop(for (batch in test_loader) {
    ...
    y <- batch[[2]]
    o <- model(x)
    yest <- torch::torch_max(o, dim = 2)[[2]]
    n_tests <- n_tests + y$shape
    n_tests_ok <- n_tests_ok + torch::torch_sum(y == yest)$item()
  })
