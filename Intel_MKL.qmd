...
Nous allons d'abord installer les librairies **Intel MKL** (processeur Intel nécéssaire) & **Open Blas**. Ces deux librairies d'algèbre linéaire permettent de d'optimiser et/ou de paralléliser un certain nombre d'opérations algébriques, notamment le calcul matriciel.
...
Nous allons comparer les performances de toutes ces librairies sur Ubuntu avec un ordinateur à 8 coeurs Intel I7. Pour effectuer ce benchmark nous devons réitérer les fermetures de sessions donc le même code a été lancé séquentiellement.
...
1. **OpenBlas** utilise constamment l'ensemble des coeurs disponibles, ce comportement extrême peut devenir un problème lors des longs calculs où lorsque l'on utilise des librairies plus complexes comme par exemple `Rcpp` avec laquelle il obtient des performances largement plus faible que la base.
1. Nous avons aussi remarqué une certaine variation dans les performances de **Intel-MKL** sur Ubuntu seulement. Bien que toujours observée plus efficace que la base. Il est fréquent que pour un même calcul le temps soit jusqu'à 4 fois plus long que le minimum. Cette variation semble être fixe pour chaque session R ouverte. Dans ces graphiques, nous appelons "lasy intel mkl" les sessions de faibles performances.
