...
On vient de créer notre premier circuit. La fonction ```circos.track``` peut prendre en entrée d'autre paramètre afin de changer la bordure des régions, le couleur de fond etc. 
...
Les fonctions graphiques par circuit sont les fonctions qui commencent par  ```circos.track...()``` comme par exemple ```circos.trackHist()```,```circos.trackLines()```,```circos.trackText()``` et marchent comme les fonctions de ```plot``` habituelle. Cependant, si des facteurs sont mis en entrées alors les données seront automatiquement séparé selon les secteurs.
