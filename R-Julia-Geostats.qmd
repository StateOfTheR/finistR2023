...
#import data
meuse = CSV.read(joinpath(current_directory,"meuse.txt"), delim="," ,DataFrame)
meuse_grid = CSV.read(joinpath(current_directory, "meuse.grid.txt"), delim="," ,DataFrame)
...
```{julia}
#Export Kriging
GeoTables.save(joinpath(current_directory,"Kriging2.shp"), Ω,force=true )

#read kriging
shape =  Shapefile.Handle(joinpath(current_directory,"Kriging2.shp")).shapes
# read grid 
grid_shp =  Shapefile.Handle(joinpath(current_directory,"boundary1.shp")).shapes
