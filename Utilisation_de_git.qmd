...
- status : affiche des informations sur la situation de notre git (la branche surlaquelle on est, les commits en cours,...) 
...
- pull : incorpore les modifications du dépot distant dans le répertoire local.
  ...
  - pull --rebase
  - pull --merge 
- config : affiche les configurations de notre git 
  - config -l 
- rebase --continue 
- log
  - log 
...
Une solution basée sur la création de différentes branches et des commandes `git stash` et `git pop`a émergé au moment de la restitution et sera testée dans un futur proche.
