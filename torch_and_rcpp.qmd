...
La librairie `BLAS` de `OpenBlas` présenté lors de cette même session ne fonctionne pas correctement avec Rcpp. On observe une baisse de fonctionnement **très quantitative** (facteur 100). La librairie `BLAS` de `Intel MKL` reste quant à elle plus efficace.
