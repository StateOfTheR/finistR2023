...
L'utilisation de {Rcpp} permet d'exporter des fonctions `C++` en R. Les fonctions seront alors directement utilisable dans un script et avec des arguments R. Ainsi on peu tirer parti de la compilation d'un code `C++`, et accélérer de nombreux calculs algébriques.
...
La librairie `BLAS` de `OpenBlas` présenté lors de cette même session ne fonctionne pas correctement avec Rcpp. On observe une baisse d'efficacité **très quantitative** (facteur 100). La librairie `BLAS` de `Intel MKL` reste quant à elle plus efficace.
